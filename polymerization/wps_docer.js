/*
    ...
    环境变量名：无
    环境变量值：无
    备注：WPS稻壳版签到，接口 welfare.docer.wps.cn。签到成功后领取每日PPT，可选转存到金山文档。
          配合“wps”分配置表使用（cookie 即 wps_sid）。
          “wps”分配置表列：A=cookie，B=是否执行，C=账号名称，
          D=转存PPT(稻壳版用，是否自动保存到金山文档)，E=是否渠道1打卡，F=是否渠道2打卡，G=Signature(渠道2)
          说明：本脚本读取列D（转存PPT），填“是”则在领取PPT后自动转存到金山文档。
*/

var sheetNameSubConfig = "wps"; // 分配置表名称（共享的分表，subConfigWps 的键）
let sheetNameSubConfig2 = "wps_docer"; // 在 CONFIG 表中的唯一名称，用于读取个性化配置与写入消息队列
var pushHeader = "【WPS稻壳版】"; // 推送头，给自己看的
...
function execHandle(cookie, pos) {
  ...
  try {
    let flagSign = 0; // 签到标志
    let url = "https://welfare.docer.wps.cn/sign_in/v1/user_sign_in"; // 签到
    let url2 = "https://welfare.docer.wps.cn/common/v1/sign_base_info"; // 获取pptid
    let url3 = "https://welfare.docer.wps.cn/sign_in/v1/sign_in_mb/receive_mb_today"; // 领取ppt
    let url4 = "https://welfare.docer.wps.cn/common/v1/mb_download"; // 保存ppt到金山文档中
    ...
    if (resp != null && resp.status == 200) {
      try {
        ...
        if (result == "ok") {
          messageSuccess += "🎉 签到成功\n";
          flagSign = 1;
        } else if (msg == "had sign in" || msg.indexOf("已经") >= 0) {
          ...
          if (resp.status == 200) {
            try {
              ...
              if (result == "ok") {
                ...
                console.log(msg);
                messageFail += "❌ 领取ppt失败：" + msg + "\n";
              }
              console.log(resp);

              // 是否自动转存ppt到金山文档（列D）
              let flagSave = Application.Range("D" + pos).Text;
              if (flagSave == "是") {
                ...
                if (resp.status == 200) {
                  try {
                    ...
                    if (result == "ok") {
                      ...
                      console.log(msg);
                      messageFail += "❌ 保存ppt失败：" + msg + "\n";
                    }
                    console.log(resp);
                  } catch {
                    console.log(resp.text());
                    console.log("保存ppt失败");
                    messageFail += "❌ 保存ppt失败：响应解析错误\n";
                  }
                } else {
                  console.log(resp.text());
                  messageFail += "❌ 保存ppt失败：HTTP " + resp.status + "\n";
                }
              } else {
              ...
              console.log(resp.text());
              console.log("领取ppt失败");
              messageFail += "❌ 领取ppt失败：响应解析错误\n";
            }
          } else {
            console.log(resp.text());
            messageFail += "❌ 领取ppt失败：HTTP " + resp.status + "\n";
          }
        } catch {
          console.log(resp.text());
          console.log("获取ppt_id失败");
          messageFail += "❌ 获取ppt_id失败\n";
        }
      } else {
        console.log(resp.text());
        messageFail += "❌ 获取ppt_id失败：HTTP " + resp.status + "\n";
      }
    }
